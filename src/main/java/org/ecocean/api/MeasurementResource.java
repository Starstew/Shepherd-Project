 ...
 * @author mmcbride
 */
@Path("/measurements/{id}.json")
public class MeasurementResource {
  @GET
  @Produces("application/json")
  public Measurement getMeasurement(@PathParam("id") String id) throws Exception {
    ...
    if (o instanceof Collection) {
      Collection<Measurement> candidates = ((Collection<Measurement>)o);
      if (candidates.size() > 0) {
        return candidates.iterator().next();
      } else {
        throw new NotFoundException("no measurements matching id " + id);
      }
    } else {
